...
def get_real_request() -> bytes:
    ...
    return signer._concat_data_to_sign(
...
def test_real_concat() -> bytes:
    data = get_real_request()

    with open("tests/data/real_data_to_sign.bin", "rb") as f:
        data_verify = f.read()

    assert data == data_verify
